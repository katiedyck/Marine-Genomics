...
rl_data <- read_tsv("../workshop3/read_lengths.txt", col_names = c("id", "length"), show_col_types = FALSE)
...
Create a new shell script, `run_flye.sh`, in the `flye` directory with the following code:
...
```{r assembly fasta bandage fig, echo=FALSE, out.width="90%"}
knitr::include_graphics("../workshop3/images/assembly_fasta.png")
```

**Observations**:

-   Largest contig at the top (contig 29) = 4,681,059 bp; depth 21x. Looks like this contig corresponds to the genome of the most abundant species in our mock community.
-   Contig 2 (top left corner) could also be a genome from our mock community (3,179,384 bp; depth 15x).
-   Contig 29 and 42 are chromosomes - we know this because they are circular (i.e. connected to themselves).
-   Contig 38 (*tall purple contig connected to itself and contiguous 39, second row from the bottom*) must be a repeat - it has a length of only 521 bp but a depth of 769x!
-   Contig 10 (*third row, connected to itself*) is interesting: 320,081 bp, depth 14x.
...
Make a new text file called "gc.awk" with the code in the chunk below and save it in the top level of the project directory (`workshop3` folder in this case).
...
```{bash ref genomes mock, eval=FALSE}
...
**In the assembly graph diagram above, the two circular chromosomes are for *Vibrio alfacsensis*.**

## Step 3: Basic Assembly QC

Here we will print the length and GC content for our assembled contigs.

```{bash basic assembly QC, eval=FALSE}
...
```{r gc and info, results='hide'}
library(tidyverse)

gc <- read_tsv("../workshop3/assembly_check/assembly.gc.tsv",col_names = c("#seq_name","length","GC"))
info <- read_tsv("../workshop3/flye/mock/assembly_info.txt")
```
...
| MetaBAT 2 (nosha) using minContig 2500, minCV 1.0, minCVSum 1.0, maxP 95%, minS 60, maxEdges 200 and minClsSize 200000. with random seed=5
| 5 bins (13398550 bases in total) formed.

The `bins` directory (in the metabat directory) will contain multi-fasta files with each of the bins inferred by metabat.

```{bash grep bins, eval=FALSE}
# quickly inspect which contigs were grouped into which bin
grep '>' bins/*
```

| bins/metabat.1.fa:\>contig_18
| bins/metabat.1.fa:\>contig_11
| bins/metabat.1.fa:\>contig_20
| bins/metabat.2.fa:\>contig_8
| bins/metabat.2.fa:\>contig_28
| bins/metabat.2.fa:\>contig_1
| bins/metabat.2.fa:\>contig_19
| bins/metabat.3.fa:\>contig_31
| bins/metabat.3.fa:\>contig_30
| bins/metabat.3.fa:\>contig_41
| bins/metabat.3.fa:\>contig_3
| bins/metabat.3.fa:\>contig_5
| bins/metabat.3.fa:\>contig_6
| bins/metabat.3.fa:\>contig_12
| bins/metabat.4.fa:\>contig_15
| bins/metabat.4.fa:\>contig_25
| bins/metabat.4.fa:\>contig_34
| bins/metabat.4.fa:\>contig_17
| bins/metabat.4.fa:\>contig_27
| bins/metabat.4.fa:\>contig_26
| bins/metabat.4.fa:\>contig_37
| bins/metabat.4.fa:\>contig_22
| bins/metabat.4.fa:\>contig_23
| bins/metabat.4.fa:\>contig_33
| bins/metabat.4.fa:\>contig_4
| bins/metabat.4.fa:\>contig_24
| bins/metabat.5.fa:\>contig_35
| bins/metabat.5.fa:\>contig_16
| bins/metabat.5.fa:\>contig_21
| bins/metabat.6.fa:\>contig_2
| bins/metabat.6.fa:\>contig_42
| bins/metabat.7.fa:\>contig_10
| bins/metabat.7.fa:\>contig_9

metabat has grouped contigs into 7 bins (exact numbers can vary from run to run which is why we used the `--seed` argument). If you compare the contigs in these bins with those shown in bandage you can see that *the largest and highest coverage contig (contig 29) has not been included*. Other than this some of the other bins seem to make some sense. (For example you should see bins that contain similar groupings of contigs to those that you explored by hand using bandage + GC content and coverage.)
...
```{bash make bins dir, eval=FALSE}
# prepare inputs for checkM - copy over the bins from metabat
mkdir bins
cp ../metabat/bins/metabat.*.fa bins/
```

Remember that metabat failed to include our longest contig in bins (contig_29). We need to extract this contig from our assembly and include it with our metabat bins.

```{bash samtools, eval=FALSE}
samtools faidx ../flye/mock/assembly.fasta contig_29 > bins/contig_29.fa
```

In `checkm` directory, save a new shell script for us to run (`run_checkm.sh`; code below). We do this because `checkm` can be very resource intensive.
...
|-----------|------------------------------------|--------------|---------------|
| metabat.7 | f\_\_Vibrionaceae (UID4865)        | 99.93        | 1.33          |
| contig_29 | p\_\_Cyanobacteria (UID2182)       | 99.87        | 0.71          |
| metabat.6 | c\_\_Deltaproteobacteria (UID3217) | 43.19        | 0.00          |
| metabat.5 | c\_\_Deltaproteobacteria (UID3217) | 29.14        | 0.00          |
| metabat.4 | f\_\_Vibrionaceae (UID4865)        | 19.13        | 0.00          |
| metabat.3 | root (UID1)                        | 12.50        | 0.00          |
| metabat.2 | k\_\_Bacteria (UID203)             | 9.64         | 0.00          |
| metabat.1 | f\_\_Vibrionaceae (UID4865)        | 7.48         | 0.00          |

**Observations**

-   The bin with the highest contamination level is *metabat.7*. Contig 42 has a length of 1,553,715 bp and a depth of 15x; contig 10 has a length of 320,088 bp and a depth of 14x (high change of repetition); contig 43 has a length of 221,077 bp and a depth of 15x (high chance of repetition).
